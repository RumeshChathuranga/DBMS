        ...
        <div className="flex gap-2">
          ...
          {booking.bookingStatus === "CheckedOut" && (
            <Button
              ...
              variant="secondary"
              onClick={() =>
                navigate(`/billing?bookingId=${booking.bookingID}`)
              }
          ...
          <Card className="bg-purple-50 border-purple-200">
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-purple-900">
                {
                  bookings.filter((b) => b.bookingStatus === "CheckedOut")
                    .length
                }
              </p>
            </div>
          ...
          <form onSubmit={handleCreateBooking} className="space-y-4">
                  ...
                  {guests.map((guest) => (
                    <div
                      ...
                      className={`p-3 cursor-pointer hover:bg-gray-50 border-b ${
                        newBookingData.selectedGuest?.guestID === guest.guestID
                          ? "bg-primary-50"
                          : ""
                      }`}
                    >
              ...
              {newBookingData.selectedGuest && (
                <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-sm font-medium text-green-900">
                    Selected: {newBookingData.selectedGuest.firstName}{" "}
                    {newBookingData.selectedGuest.lastName}
                  </p>
                </div>
              )}
            </div>
                ...
                >
                  ...
                  {roomTypes.map((type) => (
                    <option key={type.typeID} value={type.typeID}>
                      {type.typeName} - LKR {type.currRate.toLocaleString()} /
                      night
                    </option>
                  ))}
                </select>
            ...
            <Input
              ...
              onKeyDown={(e) => {
                ...
                // Allow arrow step, navigation, editing, and common shortcuts
                if (
                  key === "ArrowUp" ||
                  key === "ArrowDown" ||
                  key === "ArrowLeft" ||
                ...
                // Block anything else (e.g., e, -, +, .)
                e.preventDefault();
              }}
              onChange={(e) => {
                const next = parseInt(e.target.value || "0", 10);
                const safe = Number.isNaN(next) ? 1 : Math.max(1, next);
                setNewBookingData({ ...newBookingData, numGuests: safe });
              }}
              required
            />
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {selectedBooking.branchLocation}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      LKR {selectedBooking.rate.toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
                ...
                {selectedBooking.bookingStatus === "CheckedOut" && (
                  <Button
                    ...
                    onClick={() => {
                      setShowDetailsModal(false);
                      navigate(
                        `/billing?bookingId=${selectedBooking.bookingID}`
                      );
                    }}
