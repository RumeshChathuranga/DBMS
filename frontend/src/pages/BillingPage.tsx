...
const BillingPage = () => {
  ...
  const handleSearchInvoice = async () => {
    ...
    try {
      ...
      if (booking.bookingStatus !== 'CheckedOut' && booking.bookingStatus !== 'CheckedIn') {
        toast.warning('Invoice can only be generated for checked-in or checked-out bookings');
        return;
      }

      // Try to get existing invoice or generate new one
      try {
        const invoiceData = await billingService.getInvoiceByBooking(Number(bookingSearch));
        setInvoice(invoiceData);
        
        // Get payments
        const paymentsData = await billingService.getPaymentsByInvoice(invoiceData.invoiceID);
        setPayments(paymentsData);
      } catch (error: any) {
        if (error.response?.status === 404) {
          // Generate invoice if it doesn't exist
          await billingService.generateInvoice(Number(bookingSearch));
          const invoiceData = await billingService.getInvoiceByBooking(Number(bookingSearch));
          setInvoice(invoiceData);
          setPayments([]);
      ...
      <div className="space-y-6">
        ...
        <Card>
          <div className="flex gap-4">
            ...
            <Button variant="primary" onClick={handleSearchInvoice} loading={loading}>
              Search
            </Button>
          </div>
        </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="lg:col-span-2">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Invoice #{invoice.invoiceID}</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between py-2 border-b">
                    ...
                    <span className="font-medium">LKR {invoice.roomCharges.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b">
                    <span className="text-gray-600">Service Charges:</span>
                    <span className="font-medium">LKR {invoice.serviceCharges.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b">
                    <span className="text-gray-600">Tax Amount:</span>
                    <span className="font-medium">LKR {invoice.taxAmount.toLocaleString()}</span>
                  </div>
                  {invoice.discountAmount > 0 && (
                    <div className="flex justify-between py-2 border-b text-green-600">
                      <span>Discount:</span>
                      <span className="font-medium">- LKR {invoice.discountAmount.toLocaleString()}</span>
                    </div>
                  )}
                  <div className="flex justify-between py-3 border-t-2 border-gray-800">
                    <span className="text-lg font-bold">Total Amount:</span>
                    <span className="text-lg font-bold">LKR {invoice.totalAmount?.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-2 bg-blue-50 px-3 rounded">
                    <span className="text-blue-900">Amount Paid:</span>
                    <span className="font-medium text-blue-900">LKR {invoice.settledAmount.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-2 bg-red-50 px-3 rounded">
                    <span className="text-red-900 font-medium">Balance Due:</span>
                    <span className="font-bold text-red-900">LKR {invoice.balanceDue?.toLocaleString()}</span>
                  </div>
                </div>
              ...
              {payments.length === 0 ? (
                ...
                <div className="space-y-3">
                  {payments.map((payment) => (
                    <div key={payment.transactionID} className="p-3 bg-gray-50 rounded-lg">
        ...
        >
          <form onSubmit={handleProcessPayment} className="space-y-4">
            ...
            <div className="flex gap-3 pt-4">
              <Button type="submit" variant="success" className="flex-1" icon={<DollarSign size={20} />}>
                Confirm Payment
              </Button>
