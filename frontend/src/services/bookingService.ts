...
class BookingService {
  ...
  async checkIn(id: number): Promise<ApiResponse> {
    ...
    try {
      const result = await api.post<ApiResponse>(`/reservations/${id}/check-in`);
  ...
  async checkOut(id: number): Promise<ApiResponse> {
    ...
    try {
      const result = await api.post<ApiResponse>(`/reservations/${id}/check-out`, {});
  ...
  async checkAvailability(
    ...
    console.log('Checking availability:', { branchID, typeID, checkInDate, checkOutDate });
  ...
  async getRecentActivities(limit: number = 10, branchId?: number): Promise<any[]> {
