...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/auth/reset-password`, {
        email,
        otp,
      ...
      toast.error(e.response?.data?.detail || 'Reset failed. Check OTP and try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-white p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-200 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Input label="OTP" type="text" required value={otp} onChange={e=>setOtp(e.target.value)} placeholder="6-digit code" maxLength={6} />
